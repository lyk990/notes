...
```javascript
// 主要依赖于createApp（App）和mount（rootContainer）传入一个根组件和根容器。基于根组件会生成一个虚拟节点（var vnode = createVNode(rootComponent)）,在通过render(vnode , rootContainer) 将其渲染出来，最后会将mount方法return出去。
